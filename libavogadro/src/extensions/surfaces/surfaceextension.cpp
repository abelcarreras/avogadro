...
namespace Avogadro
{
  ...
  void SurfaceExtension::calculateFracOccupDensity(OpenQube::Cube *cube) {
      if (!m_basis)
          return;
      ...
      // Set up the progress bar
      m_progress->setWindowTitle(tr("Calculating fractional occupation density"));
      m_progress->setRange(m_basis->watcher().progressMinimum(),
                           m_basis->watcher().progressMaximum());
      m_progress->setValue(m_basis->watcher().progressValue());
      m_progress->show();
                                              ...
                                              bool &calculateCube)
  {
    switch (type) {
      ...
      case Cube::FracOcupDensity: {
        ...
        if (!cube) { // We need a new cube
            cube = newCube();
            cube->setName(tr("Frac. Occ. Density"));
            cube->setCubeType(Cube::SpinDensity);
            m_cubes[2] = cube->id();
            m_cube = cube;
            m_qube = newQube();
            calculateFracOccupDensity(m_qube);
            calculateCube = true;
            return;
        }
            // There is a valid cube - check the resolution
    ...
    switch (m_calculationPhase) {
      case 0: { // main cube was calculated - possibly kick off a color cube
        ...
        if (m_surfaceDialog->cubeType() == Cube::MO ||
            ...
            m_surfaceDialog->cubeType() == Cube::FracOcupDensity ||
