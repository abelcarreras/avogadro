...
void GaussianFchk::processLine()
{
  ...
  //else if (key == "Alpha NATO coefficients") {
    ...
    if (readSpinDensityMatrix(list.at(2).toInt(), 16))
      ...
      if (readFODDensityMatrix(list.at(2).toInt(), 16))
          qDebug() << "Fractional occupation density matrix read in" << m_foddensity.rows();
      else
          qDebug() << "Error reading in the fractional occupation density matrix.";
  }
}
...
bool GaussianFchk::readFODDensityMatrix(unsigned int n, int width)
{
    // This function reads in the lower triangular density matrix
    m_foddensity.resize(m_numBasisFunctions, m_numBasisFunctions);
    unsigned int cnt = 0;
    unsigned int i = 0, j = 0;
    unsigned int f = 1;
    bool ok = false;
    while (cnt < n) {
        ...
        if (width == 0) { // we can split by spaces
            ...
            for (int k = 0; k < list.size(); ++k) {
                ...
                // Read in lower half matrix
                m_foddensity(i, j) = list.at(k).toDouble(&ok);
                if (ok) { // Valid double converted, carry on
                    ++j; ++cnt;
                    if (j == f) {
                        // We need to move down to the next row and increment f - lower tri
            ...
            for (int c = 0; c < maxColumns; ++c) {
                ...
                // Read in lower half matrix
                m_foddensity(i, j) = substring.toDouble(&ok);
                if (ok) { // Valid double converted, carry on
                    ++j; ++cnt;
                    if (j == f) {
                        // We need to move down to the next row and increment f - lower tri
